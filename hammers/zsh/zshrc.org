...
**** //usr/local/bin/
     #+BEGIN_SRC shell :tangle (m/resolve "${m/xdg.conf.d}/zsh/init/usr_local_bin.zsh") :eval never :exports code :comments link
...
**** coreutils gnu tools path  
     #+BEGIN_SRC shell :tangle (or (and (eq m/os 'macos) (m/resolve "${m/xdg.conf.d}/zsh/post/loads.zsh")) "no") :eval never :exports code :comments link
       export PATH="$(find -L /usr/local/opt -maxdepth 3 -mindepth 3 -type d -name gnubin | paste -s -d: -):$PATH"
     #+END_SRC
