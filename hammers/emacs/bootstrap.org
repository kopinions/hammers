...
*** 包管理器配置
    ...
    #+BEGIN_SRC elisp :eval never :exports code :tangle (m/resolve "${m/conf.d}/init.el") :comments link
      ...
      (let* ((verilog-lib (locate-library "verilog-mode")))
	(if verilog-lib
	    (if (string-match ".*/lisp/progmodes/.*" verilog-lib)
		(let* ((verilog-lib-dir (replace-regexp-in-string "\\(.*/lisp/progmodes\\).*" "\\1" verilog-lib)))
		  (progn
		    (message verilog-lib-dir)
		    (delete-file (expand-file-name "verilog-mode.elc" verilog-lib-dir ))
		    (delete-file (expand-file-name "verilog-mode.el.gz" verilog-lib-dir)))))))

      (mapatoms (lambda (s)
		  (let ((sn (symbol-name s)))
		    (when (string-match "^\\(verilog\\)\\(-.*\\)?$" sn)
		      (setplist s nil)
		      (unintern s)
		      ))))
