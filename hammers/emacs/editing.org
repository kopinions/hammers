...
** 数字增加
   #+BEGIN_SRC elisp :eval never :exports code :tangle (m/resolve "${m/conf.d}/lisp/init-editing.el") :comments link
     (defun m/increment-number-hexadecimal (&optional arg base)
       ...
       (save-excursion
	 (save-match-data
	   (let (inc-by field-width answer hex-format hex-en repeat-cnt prompt base skipchars regx)
	     (setq inc-by 1)
	     (setq repeat-cnt 1)
	     (setq base 10)
	     (setq skipchars "0123456789")
	     (setq regx "\\([0-9]+\\)")
	     (cond
	      ((listp arg) (cond
			    ((eq (car arg) 4)  (progn
						 ...
						 (setq skipchars "0123456789")
						 (setq regx "\\([0-9]+\\)")
						 (setq inc-by (string-to-number (read-string prompt) base))
						 (setq repeat-cnt (string-to-number (read-string "repeat:")))
			    ...
			    ((eq (car arg) 16)  (progn
						  ...
						  (setq prompt "hex:")
						  (setq base 16)
						  (setq skipchars "0123456789abcdefABCDEF")
						  (setq regx "\\(0[xX]\\)\\([0-9a-fA-F]+\\)")
						  (setq inc-by (string-to-number (read-string prompt) base))
						  (setq repeat-cnt (string-to-number (read-string "repeat:")))
						  (skip-chars-backward skipchars)
						  (re-search-forward regx nil t)))))
	      ...
	      ((eq arg '-) (progn
			     ...
			     (setq repeat-cnt 1))))
	     (skip-chars-backward skipchars)
	     (let* ((group (if hex-en 2 1)))
	       (when (re-search-forward regx nil t)
		 (setq field-width (length (match-string group)))
		 (setq answer (+ (string-to-number (match-string group) base) (* repeat-cnt inc-by)))
		 (when (< answer 0)
		   (setq answer (+ (expt 16 field-width) answer)))
		 (if (equal (match-string 0) (upcase (match-string 0)))
		     (setq hex-format "X")
		   (setq hex-format "x"))
		 (replace-match (format (if hex-en (concat "%0" (int-to-string field-width)
							   hex-format) (concat "%0" (int-to-string field-width)
							   "d")) answer))))))))
