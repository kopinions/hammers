...
**** pre hook
     #+BEGIN_SRC shell :eval never :exports code :tangle (m/resolve "${m/xdg.conf.d}/notmuch/default/hooks/pre-new") :tangle-mode (identity #o755) :shebang #!/bin/bash :noweb yes :comments link
       ...
       echo moving $(notmuch count --output=files --exclude=false tag:gb2 AND folder:'ding/INBOX' AND NOT tag:unread AND NOT from:neo) gb2 messages to the GB2
       for i in $(notmuch search --exclude=false --output=files tag:gb2 AND folder:'ding/INBOX' AND NOT tag:unread AND NOT from:neo); do
       ...
       # done
       mbsync -c ${XDG_CONFIG_HOME}/isync/config ding-inbox ding-drafts ding-sent ding-spam ding-archive ding-gb2 ding-patent 
